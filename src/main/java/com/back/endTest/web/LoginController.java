  ...
  public String login(
    ...
    if (loginType.equals("person")) {
      ...
      if (!memberPersonSVC.isExist(loginPersonForm.getIdPerson())) {
        bindingResult1.rejectValue("idPerson", "loginPage", "아이디가 없네유");
        return "loginPage";
      }
      //2)로그인
      ...
      if (memberPerson.isEmpty()) {
        bindingResult1.rejectValue("pwPerson", "loginPage", "비밀번호가 서로 다릅니다~");
        return "loginPage";
      }
      //3)세션 생성
    ...
    if (loginType.equals("company")) {
      ...
      if (!memberCompanySVC.isExist(loginCompanyForm.getIdCompany())) {
        bindingResult2.rejectValue("idCompany", "loginPage", "아이디가 없네유");
        return "loginPage";
      }
      //2)로그인
      ...
      if (memberCompany.isEmpty()) {
        bindingResult2.rejectValue("pwCompany", "loginPage", "비밀번호가 서로 다릅니다~");
        return "loginPage";
      }
      //3)세션 생성
      HttpSession session = httpServletRequest.getSession(true);
